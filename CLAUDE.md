...
- **Site config**: `_config.yml` holds identity (`name`, `url`, `repository`), the sidebar `author:` block (avatar image in `images/`, social/academic profile links), theme choice (`site_theme`), publication categories, and front-matter `defaults` per collection. The header menu order lives in `_data/navigation.yml`; its `CV` entry points straight at `/files/Emile_Emery_CV_EN.pdf` rather than a page.
